 ...
 "use strict";
    class DraftViewViewModel {
        constructor(args) {
            ...
            self.DraftInvoiceDet = ko.observableArray([]);
            ...
            function getUnpublishInvoice(){
                document.getElementById('loaderView').style.display='block';
                self.DraftInvoiceDet([]);
                $.ajax({
                    url: BaseURL  + "/jpGetDraftInvoiceList",
                    type: 'POST',
                    data: JSON.stringify({
            ...
            self.dataProvider = new ArrayDataProvider(this.DraftInvoiceDet, { keyAttributes: "id"});

            self.viewDraftInvoice  = function (event,data) {
                ...
                self.router.go({path:'viewDraftInvoice'})
            }
