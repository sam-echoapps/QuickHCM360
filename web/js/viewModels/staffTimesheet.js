define(['ojs/ojcore', 'knockout', 'jquery', 'appController', "ojs/ojmodulerouter-adapter", "ojs/ojarraydataprovider", 'ojs/ojknockout-keyset', "ojs/ojlistitemlayout",
                    ...
                    success: function (result) {
                      console.log(result)
                       ...
                       self.gender(data[4])
                        ...
                        imgElement.src = 'data:image/jpeg;base64,' + result[14][0];
    ...
    self.timesheetPDFGenerate = function (event,data) {
       ...
       /*  var disp_setting="toolbar=yes,location=no,";
        ...
        disp_setting+="scrollbars=yes,width=650, height=600, left=100, top=25";
           var content_vlue = document.getElementById('PDFDownload').innerHTML;
        ...
        var printContents = document.getElementById('PDFDownload').innerHTML;
    ...
    //    document.body.innerHTML = originalContents;
       ...
       self.approveTimesheet = function (event,data) {
        var clickedRowId = data.data.timesheetId
            document.querySelector('#openTimesheetApproveProgress').open();
            $.ajax({
                url: BaseURL + "/jpSatffTimesheetApprove",
                type: 'POST',
                data: JSON.stringify({
                    rowId : clickedRowId,
                    loggedUser : sessionStorage.getItem("userName")
                ...
                error: function (xhr, textStatus, errorThrown) {
                    if(textStatus == 'timeout'){
                        document.querySelector('#openTimesheetApproveProgress').close();
                        document.querySelector('#Timeout').open();
                    }
                },
                success: function (data) {
