...
function (oj,ko,$, app, ojconverterutils_i18n_1, ArrayDataProvider,  ojknockout_keyset_1,ResponsiveUtils, ResponsiveKnockoutUtils, AsyncRegExpValidator) {
 "use strict";
    class ClientPaidInvoiceViewModel {
        constructor(args) {
            var self = this
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            self.PaidInvoiceDet.push({'id': data[i][0],'serial_number': "INV"+(8000+data[i][0]),'start_date': data[i][1], 'end_date': data[i][2], 'grand_total': data[i][3], 'invoice_date': data[i][4], 'payment_due_date': data[i][5]});
                    }
                    }
                })
    ...
    return ClientPaidInvoiceViewModel;
});